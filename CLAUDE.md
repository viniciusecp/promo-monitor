...
- `app/workers/supervisor.py` owns the pipeline. `ensure_started()` is idempotent (guarded by a lock *and* by `MessageListener._handler_registered`) — a double registration means every message is processed twice, i.e. duplicated matches and alerts. `refresh_interests`, the watchdog and the pause task are **stored** tasks, cancelled in `stop()`.
- **Zero active interests pauses the capture at the network level.** `_set_capture(False)` stops the listener and calls `client.set_receive_updates(False)` (`app/telegram/client.py`), so Telegram stops pushing messages altogether. Three things make this work and are easy to break:
  - The flag lives in the **module**, not on the client: `reset_client()` throws the object away, and any rebuild (the watchdog's included) must keep honouring the mode or the firehose reopens silently.
  - It **reconnects**. Telethon's `client.set_receive_updates()` does not stop an established stream — the flag only wraps `initConnection` at `connect()` time (`telegrambaseclient.py`) and, at runtime, outgoing requests (`users.py`). Only a reconnect actually silences it.
  - The `MessageListener` holds the client reference, so it is **rebuilt** after every mode switch. The watchdog takes the same `_capture_lock` — a tick landing mid-switch would reconnect the discarded client and undo the pause.
  - Pausing waits `_PAUSE_GRACE` (30s) because editing a set of interests passes through zero-active states and each transition costs a reconnect; resuming is immediate. Resuming does **not backfill**: `MessageListener.capture_since` drops anything older, since the reconnect's `GetDifference` would otherwise replay the deaf window and alert on stale promos. **That cutoff is for resume only** — `_build()` passes `capture_since=None` at boot. A restart is not a deliberate deaf window: Telethon persists MessageBox state in the session file and `_update_loop` calls `get_difference()` regardless of `catch_up`, so the backlog of a `docker compose up --build` or a `restart: on-failure` cycle *is* redelivered, and it is exactly what must not be dropped. `_set_capture`'s default is therefore `None` (process everything) — a call site that forgets the argument duplicates at worst, never loses.
  - `/health` therefore exposes `capture_active` and `interests_count` next to `worker_running` — the worker stays up while paused, so `worker_running` alone would claim the capture is running with the stream cut.
